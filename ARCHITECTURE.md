...
- **Current repository truth:** `main` contains the committed r2 implementation through merge `5376b51` (implementation `297b2fd`). The clean r3 planning branch is `brief/p0-005-r3-cohesive-routine-management` at `b0b966e`. Multi-routine definitions, dayparts, archive, and scoped personalization exist. r2 adds `occurrences.started_at`, same-date revision upsert, per-person locks, and pending-first-action protection on Today refresh. Migrations end at `006_occurrence_structural_lock.sql`. Save explicitly reconciles only its exact date; other unstarted rows may reconcile on read. There is no upcoming-change lifecycle or routine deletion UI. The editor loads the latest revision and saves today. P0-005 r3 below is planned, not implemented. Recorded pre-P0-005 integrated baseline: `93ef494`.
...
The r1/r2 branch implements this foundation. The authoritative next contract is `briefs/p0-005-multiple-household-routines.md` revision 3. D-020/D-021 retain identity/daypart conventions; D-024–D-026 describe planned r3 changes. Older milestone narratives below are historical context where a later decision explicitly refines them.
...
- New routines begin today if their weekday applies. r2 shared saves default to today and upsert that date, while personal writes remain tomorrow-or-later. r3 replaces exposed revision handling with current-plan and intentional-upcoming operations. Current/upcoming summaries must resolve the applicable plan and dated group set together.
...
- Archive removes a routine from the active list immediately and stops participation from tomorrow's household date. Keep today/history and valid delayed reports before cutoff. Retained future rows are not actionable, including after their dates pass. Intersect reference intervals with archive cutoff. Archived details/history remain inspectable; new edits/personal changes/approvals are blocked, pending proposals remain auditable/rejectable, and restore is deferred.
...
### P0-005 r3 planned routine lifecycle and application design

D-024–D-026 and the r3 brief define the next work. Product's supplied proposal is recorded in PRODUCT.md. These requirements deliberately extend r2; prior acceptance is not evidence that they already work.
...
- **Upcoming lifecycle:** Support ordered future entries with stable identity. Create/edit/move/delete through normal UI; editing retains the chosen date, moving re-resolves vacated/new ranges, deleting makes the predecessor continue until the next remaining boundary. A date collision is recoverable and never silently shifts or overwrites a change.
- **Shared resolver:** Plan selection, recurrence, dated groups, personal composition, preview, cached execution, History, and used-by references must agree. Return started rows independently of newer plan eligibility, so removing a person/weekday cannot hide protected work. Compose applicable personal layers even when the shared plan did not change.
- **Storage/replay:** Add forward migrations after 006 as needed. Preserve all protected references and migrated future dates. Keep immutable plan content versions behind active schedule selection, or an equivalent history-safe representation. Commit mutation state, affected rows, and receipt together; invalidate only after commit.
- **First action and recovery:** Validate structure identity/obligation/participation inside the transaction. An edit/action race must preserve committed locks or expose a recoverable stale-structure result. Pending local intent survives disappearance from a server list, reload, and reconnect until acknowledged or visibly resolved. Reject old response versions even after queue drain.
- **Delete/End:** Delete unused setup only after exhaustive reference checks; materialization/save receipts alone do not block. End cancels unstarted today and later plus upcoming changes, retaining started work and audit. This changes new-operation D-022 tomorrow cutoff semantics. Existing archived records keep their recorded cutoff. Minimal deletion receipts cannot resurrect deleted routine content. No restore is introduced.
- **Shell/navigation:** Phone primary navigation is labeled Today / Routines / Household, with Routines management capability-gated. Household holds People & Groups, activity, Approvals, and History under existing grants. Personalize remains reachable from Today. Centralize destination metadata/typed local state while keeping session, outbox and sync above views; no new router requirement.
- **Calm Household foundation:** Extend the existing CSS variables into typography/spacing/surface/control/status/focus tokens and a small set of presently shared primitives. Use restrained warm light surfaces, quiet rows, one accent, and 44px hit areas. Apply across current execution, routine, household, personal and auth surfaces. Local and release share the theme; an explicit Local development indicator replaces the distinct palette.
- **Evidence:** The r3 browser journey must actually schedule, move/delete upcoming changes, Delete/End routines, and exercise peer locks, future-row reconciliation, and offline conflicts. Require semantic, focus, responsive and representative screenshot evidence across the connected app; Product judges coherence. All normal validation stays local.
...
- **Persistence/retention:** Preserve protected occurrences, execution reports and personal/proposal audit. Planned D-025 permits deletion of unused routine setup and End for retained work; broader household export/retention remains TBD. Chore debt is not generated.
