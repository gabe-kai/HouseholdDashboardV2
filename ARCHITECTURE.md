...
- **Current repository truth:** Inspected clean branch `brief/p0-005-multiple-household-routines` at `297b2fd` (committed r2). Multi-routine definitions, dayparts, archive, and scoped personalization exist. r2 adds `occurrences.started_at`, same-date revision upsert, per-person locks, and pending-first-action protection on Today refresh. Migrations end at `006_occurrence_structural_lock.sql`. Save explicitly reconciles only its exact date; other unstarted rows may reconcile on read. There is no upcoming-change lifecycle or routine deletion UI. The editor loads the latest revision and saves today. P0-005 r3 below is planned, not implemented. Recorded pre-P0-005 integrated baseline: `93ef494`.
