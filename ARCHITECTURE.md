...
Authoritative contract: `briefs/p0-007c-2-household-display-read-only-dashboard.md` **revision 1**, D-043-D-045. Baseline: integrated C-1 at **ef39a89**. Engineering readiness is **READY**. Architecture reviewed implementation **a2b9e42** and correction commits **90aaa74** and **ba5541c** on 2026-09-25. The initial findings and most second-review evidence are closed. Third review remains **FIX REQUIRED** only for AT6 school-filtered-empty evidence, AT9 calendar/reassignment/parity assertions, rerunning exact `validate:pr`/`validate:rc` (including WebKit), and pinning `ba5541c` in the Build Report. The same brief revision and branch remain active.
...
- **Acceptance gaps:** Close the four third-review items recorded in the brief: prove filtered-empty omission; prove calendar exception and non-optional reassignment/C-1 parity convergence; run `validate:pr` and `validate:rc`; pin the correction SHA and truthful AT mappings in the Build Report. AT17 at-distance readability remains Project Lead evaluation.
