...
- **Current repository truth (2026-09-24):** Inspected clean integrated `main` at **ef39a89**, including technically accepted **P0-007C-1 r1** via PR **#20**, accepted A/B, migrations through **014**, assignment patterns, scheduled work, Unassigned and draft preview. Historical reports retain their original commit references across the earlier repository-history cleanup. Product evaluation remains separate. **P0-007C-2 r1** is now IN REVIEW; the display architecture below is planned, not implemented. C-3 remains the final slice of the supplied revised C proposal.
...
### P0-007C-2 Household Display and read-only dashboard (planned; IN REVIEW)

Authoritative contract: `briefs/p0-007c-2-household-display-read-only-dashboard.md` **revision 1**, D-043-D-045. Baseline: integrated C-1 at **ef39a89**. Engineering readiness is **READY** and Architecture has returned **ACCEPT / PROCEED** (2026-09-25); implementation is now authorized on the brief branch. These remain intended changes, not current application capabilities.
...
- **Implementation/evidence conventions:** Forward migrations after 014 preserve populated data, member grants/sessions and display setup through activity clear; new issuer references participate in safe fixture cleanup. Extend runtime principal guards, route-policy inventory, protected behaviors and sync matrix. Add explicit C-2 thematic CI/wall selection without weakening existing PR/RC/Vite gates. Screenshots remain ignored under `reports/_local-screenshots/p0-007c-2-r1/`; readiness/Build Reports and test assertions are tracked. Test locally; no deployment is required.
