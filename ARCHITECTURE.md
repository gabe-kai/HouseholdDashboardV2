...
- **Current repository truth:** Integrated `main` at `31aad37` includes technically accepted **P0-005 r3**. It adds `routine_schedule_entries`, immutable revision content versions, definition `ended_at` / `end_mode` / `deleted_at`, occurrence `canceled_at`, current-plan range reconciliation to the next schedule boundary, upcoming schedule create/edit/move/delete, Delete unused vs End, started/history visibility when live audience excludes a person, atomic lifecycle receipts, read-first Routines UX, Today / Routines / Household navigation, and shared Calm Household presentation. Migrations `007_routine_schedule_lifecycle.sql` and `008_widen_routine_mutation_receipt_kinds.sql` cover lifecycle storage and long-lived-database receipt kinds. r2 locks, personal tomorrow-floor, group next-day, sync/outbox, and Origin/CSRF/WS protections remain preserved. Recorded pre-P0-005 integrated baseline: `93ef494`.
- **Accepted checkout beyond local main:** Inspected `brief/p0-006a-calm-household-experience-foundation` at `183bc41` contains committed P0-006A r2, its closed local evidence gaps, and technical acceptance. The Project Lead also accepted the experience after screenshots and local use. Local `main` is still `31aad37`; remote integration has not been verified. The implemented Plan navigation, focused editors, saved URLs, and reorder described below are from this accepted checkout. P0-006B must use that foundation after Project Lead integration.
...
### P0-006B contextual routine applicability (planned; not implemented)

Authoritative contract: `briefs/p0-006b-contextual-routine-applicability.md` **revision 1**, with D-029-D-031. This is the second slice of the original P0-006 proposal. P0-006A has both technical and Project Lead acceptance; profiles/useful History remain the third slice.

- **Small domain extension:** Add a closed rule to shared steps, personal additions, and proposals: Every time, School days, No-school days, School nights, Weekdays, Weekends, Selected days. Recurrence still decides whether the routine runs. Compose stable shared/personal anchors before filtering by the rule. Preserve a child's existing direct/proposal authority and personal effective-date policy.
- **Household calendar:** One household resource with inclusive, nonoverlapping school-year ranges, usual weekdays, and no-school date/range exceptions. School nights checks the next calendar date in the household timezone. Calendar setup is required before saving school-dependent steps/personal content/proposals; unresolved preview context is explicit, never a guessed weekday calendar. New normal databases receive no calendar fixtures.
- **Authority:** Planned `household.schedule.manage` grants calendar writes; active household members may read. Manager preset and migration from existing `routine.shared.manage` receive it, without changing classification/enrollment/personal authority. Origin/CSRF, household scoping, expected version, and replay are required.
- **Retained context:** Save appends a calendar edition effective from today without date drift. Resolve the edition applicable on the occurrence date, including its School-night lookahead. Preserve existing started/past snapshots and enough edition/rule provenance to explain them; never reinterpret history using today's calendar.
- **Reconciliation:** Integrate with current plan/date/group/personal resolution. Calendar changes cross routine schedule boundaries, evaluating each date's own plan; rule edits retain D-024's interval. Reconcile affected stored unstarted work atomically with configuration and receipt; lazy reads handle unmaterialized dates. Semantic no-ops do not churn step IDs. Empty evaluated work is nonactionable, not completed and not an empty-snapshot repair target. Pending first-action intent survives list omission and exposes conflicts visibly.
- **UI and continuity:** Household -> School calendar is read-first and URL-addressable. Use A's focused draft, Back, dirty-leave, toast, and responsive controls. Applicability belongs in focused step editing; compact read rows label nondefault rules. Date-selectable previews distinguish all configured steps from evaluated execution and explain exclusions. Extend sync so context changes refresh open previews/Today even without a routine-version change.
- **Migration/evidence:** Forward migrations after 008; legacy rules default Every time, IDs/locks/audit/receipts are preserved, and routine replay digests include rules with legacy compatibility. Add a disposable populated through-008 baseline alongside existing migration fixtures. Required local evidence covers calendar truth, first-action races, all-filtered/outbox recovery, cross-device previews, and the complete school-day/school-night UI journey. No deployment or live database mutation is part of implementation.

Physical storage layout, route spelling, and helper organization remain Engineering discretion within these contracts. This section is a plan, not a claim that calendar or applicability code exists.
...
- **Pull-request validation:** `npm run validate:pr` (validate + build + Chromium e2e + Vite deep-link smoke). GitHub Actions (`.github/workflows/validate-pr.yml`) runs the same gates as separate labeled steps: Lint, Typecheck, Unit and integration tests, Production build, E2E Chromium, E2E Vite deep-link smoke.
