...
- **Shell/navigation:** Phone primary navigation is Today / Routines / Household, with Routines management capability-gated. Household holds People & Groups, activity, Approvals, and History under existing grants. Personalize remains reachable from Today. Typed local destination state; no new router.
- **Calm Household foundation:** Shared warm light tokens/primitives across execution, routine, household, personal, and auth surfaces. Local and release share the theme; an explicit Local development indicator replaces the distinct cool-dev palette.
- **Evidence:** Local `validate:pr` / `validate:rc` plus r3 screenshots under `reports/p0-005-r3-screenshots/`; see `reports/P0-005-r3-build-report.md`.

### P0-006A planned experience foundation

P0-006A revision 2 is a planned presentation and interaction slice over the accepted P0-005 domain model. Engineering readiness has not yet occurred. Contextual routine rules, profile fields, History retention changes, and new responsibility types belong to later slices.

- **Information architecture:** Refine the primary destinations to Today / Plan / Household. Plan opens Routines directly under the existing shared-management grant. The same hierarchy responds automatically across phone, tablet, narrow-window, and wide-desktop layouts; no persisted layout override or empty future destinations are introduced.
- **State ownership:** Keep session, identity, WebSocket, authoritative refresh, and checklist outbox lifetimes above the responsive shell. Visual extraction or URL-backed navigation must not remount these owners or drop pending work.
- **Saved-view identity:** Required URL views are Today, Plan/Routines list, routine detail (including retained ended routines), Household, People & Groups overview, person detail, and group detail. Use stable IDs and session authorization. Sign-in return and reload must work in Vite and the built SPA. Browser Back follows visit history while in-app Back follows parent hierarchy; dirty outer drafts are guarded across both paths. No new detail domains or synchronized/URL-encoded drafts.
- **Shared UI boundary:** Reuse present shell, page header/Back, row, action, notice, and focus conventions as needed; no exhaustive component catalog or new confirmation framework is required. Normalize actual token conflicts and inspect computed layout. Other existing screens receive shared presentation changes, not domain redesign.
- **Routine interaction:** Preserve existing create/revise/schedule APIs and stable step IDs. Read views have a bounded phone-density acceptance target. Summary sections and individual step editors stage local changes; only final Create/Save writes. Dirty drafts retain base version and target identity through sync. Reordering supports touch/pointer cancellation/autoscroll plus equivalent non-drag controls; Personalize moves additions without changing anchors or inherited-step authority.
- **Feedback:** Healthy connectivity is quiet. Offline/pending/retry/rejection stays visible, with truthful distinction between queued checklist intent and unsaved administrative drafts. Acknowledged success produces one transient toast across its own completion transition; unrelated navigation/identity change dismisses it. Errors, protected-work explanations, one-time enrollment material, and effective-date/Preview access remain usable after toast expiration.
- **Evidence:** Local PR/RC gates; bounded phone/desktop Chromium and WebKit journeys; separate touch-event, mouse and keyboard ordering evidence; measured phone density; accessibility/layout assertions; and fictional screenshots under `reports/p0-006a-r2-screenshots/`. Prior artifacts stay unchanged. Product evaluates visual coherence and physical touch feel separately.
