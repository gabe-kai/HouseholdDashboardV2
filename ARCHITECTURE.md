...
- **Current repository truth (2026-09-25):** Integrated `main` at **ef39a89** includes technically accepted **P0-007C-1 r1** via PR **#20**, accepted A/B, migrations through **014**, assignment patterns, scheduled work, Unassigned and draft preview. P0-007C-2 r1 is implemented on its brief branch through third-review evidence commit **a6d3899**; behavior and PR/RC gates are closed, but final technical acceptance is withheld only for stale Build Report commit/status metadata. It is not yet integrated. Product evaluation remains separate. C-3 remains the final slice of the supplied revised C proposal.
...
### P0-007C-2 Household Display and read-only dashboard (FIX REQUIRED — report metadata only)

Authoritative contract: `briefs/p0-007c-2-household-display-read-only-dashboard.md` **revision 1**, D-043-D-045. Baseline: integrated C-1 at **ef39a89**. Engineering readiness is **READY**. Architecture reviewed implementation **a2b9e42**, corrections **90aaa74**, **ba5541c**, and third-review evidence commit **a6d3899** on 2026-09-25. Source inspection confirms AT6 school-filtered-empty omission with neighbor retention and AT9 calendar exception, mandatory reassignment and C-1 owner/status parity. The Build Report records the required `validate:pr` and `validate:rc` gates as passing. All behavioral/test findings are closed; final acceptance is withheld solely because the Build Report still says the latest correction is uncommitted and does not pin `a6d3899`. Same r1/branch; no code or test rerun requested.
...
- **Acceptance gap:** Correct the Build Report’s stale commit/status metadata to pin `a6d389983b2f1629a202601910eb9cb42ae3c4e0` and remove pending-commit language. AT17 at-distance readability remains separate Project Lead evaluation and does not block technical acceptance.
