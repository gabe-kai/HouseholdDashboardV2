...
Authoritative contract: `briefs/p0-007c-2-household-display-read-only-dashboard.md` **revision 1**, D-043-D-045. Baseline: integrated C-1 at **ef39a89**. Engineering readiness is **READY**. Architecture reviewed implementation **a2b9e42** and correction **90aaa74** on 2026-09-25. The first-round archive/bootstrap/denial/AT1-2 findings are closed; second review remains **FIX REQUIRED** for incomplete AT3, AT5-7, AT9-15 evidence and the unpinned correction SHA in the Build Report. The same brief revision and branch remain active.
...
- **Acceptance gaps:** Finish the precise second-review evidence list in the brief, including claim transaction/replay boundaries, positive household-visible task detail, complete current-day parity, meaningful live/out-of-order/offline behavior, hosted/local cookie lifetime, browser-clock/date rendering, and zero attempted member-data requests on display deep links. AT17 at-distance readability remains Project Lead evaluation.
