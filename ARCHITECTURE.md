...
- **Current repository truth:** Integrated `main` remains `51322e0` (PR #15) with accepted **P0-007A r1** (migrations 011-013). **P0-007B r1** is implemented on `brief/p0-007b-assignment-patterns-scheduled-work` (migration **014**, assignment patterns, scheduled work, Unassigned, draft preview); evidence in `reports/P0-007B-r1-build-report.md`. C remains LIKELY NEXT.
...
Authoritative contract: `briefs/p0-007b-assignment-patterns-scheduled-work.md` **r1**, D-038-D-040. Baseline: merged A at **51322e0**. Branch `brief/p0-007b-assignment-patterns-scheduled-work`. Migration **014**, shared assignment/composition resolver, draft preview, Unassigned storage, and Kitchen/Bathroom/Cats/Trash UI journeys are implemented; Build Report is `reports/P0-007B-r1-build-report.md`. Architecture re-accepted r1 after AT2, AT7, and AT9–15 B-specific evidence closed; details: `reports/P0-007B-r1-architecture-reacceptance.md`. Card is Ready to Evaluate; product acceptance remains separate.
...
- **Pull-request validation:** `npm run validate:pr` (validate + build + Chromium e2e + Vite deep-link smoke). GitHub Actions (`.github/workflows/validate-pr.yml`) runs the same gates as separate labeled steps: Lint, Typecheck, Unit and integration tests, Production build, then split Chromium e2e (`pre-P0-007`, `P0-007A`, `P0-007B`, desktop geometry) and E2E Vite deep-link smoke. Failed runs upload `test-results/` and `playwright-report/` artifacts. Set `E2E_SKIP_BUILD=1` when the production build already ran so Playwright does not rebuild before each Chromium shard.
