...
- **Current repository truth:** Integrated `main` at `409d147` (PR #14) contains technically accepted **P0-006A r2**, **P0-006B r1**, and **P0-006C r1**, including C implementation `5605ab8` and family-order correction `e8a93d5`, through migration 010. The current P0-007A branch adds responsibility implementation and migrations 011–013 at Project Lead commit `10a412d`; Architecture re-acceptance has the responsibility corrections present but remains FIX REQUIRED because the independent suite has a date-sensitive P0-006B AT9 failure on 2026-09-20. The Project Lead accepted A after screenshots/local use and exercised B calendar setup; exhaustive C Product acceptance is not inferred.
...
### P0-007A household responsibility foundation (delivered; FIX REQUIRED)

Authoritative contract: `briefs/p0-007a-household-responsibility-foundation.md` **r1**, D-035–D-037. Base: integrated `main` **409d147**; READY review `7226987`, implementation authorization `a6890cc`, delivered implementation **10a412d**. Migrations 011–013, responsibility APIs/client, mixed views and acknowledged activity clear exist on the brief branch. Responsibility corrections are present; re-acceptance `reports/P0-007A-r1-architecture-reacceptance.md` withholds acceptance only because the independent suite currently has a date-sensitive P0-006B AT9 failure. The following bullets remain the intended contract, not a claim that every invariant has passed. First of three slices; B/C remain lighter roadmap horizons and are not newly authorized.
