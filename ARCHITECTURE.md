...
- **Current repository truth:** Integrated `main` at `de552ab` contains accepted **P0-006A r2** and **P0-006B r1**. The Project Lead accepted A after screenshots and local use. B contextual applicability was accepted after AT9 rollback injection and AT10/AT11 browser multi-context evidence closed at `8421b31`, then merged without requiring a hosted deployment.
...
- **Authority/availability:** New `household.activity.clear` is in the manager preset and backfilled only from existing `routine.shared.manage`. Enforce grant plus evaluation setting: on by default in development/test, off in hosted unless explicitly enabled with `ALLOW_EVALUATION_HISTORY_CLEAR=1`. This is planned configuration, not a current supported command or permission to deploy.
