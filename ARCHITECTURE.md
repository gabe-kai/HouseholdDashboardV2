...
- New routines begin today if their weekday applies; subsequent configuration/personal changes remain prospective. Default dates append after the target's already scheduled revisions, starting no earlier than tomorrow. Show the date before/after saving; stale writes conflict instead of silently changing the date. Current and upcoming summaries resolve the applicable revision and group set together for the displayed date.
...
- **Future participation boundary (implemented P0-004B):** Materialized structure remains durable, but participation for a date later than today is provisional. Dated group membership may change whether an existing future row is returned/actionable; it may not rewrite or delete that row's snapshot or durable execution evidence.
