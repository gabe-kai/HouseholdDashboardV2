...
- **Current repository truth:** Integrated `main` still holds committed r2 through merge `5376b51` (implementation `297b2fd`). Local **P0-005 r3** implementation lives on `brief/p0-005-r3-cohesive-routine-management`. r3 adds `routine_schedule_entries`, immutable revision content versions (no unique effective-date constraint), definition `ended_at` / `end_mode` / `deleted_at`, occurrence `canceled_at`, current-plan **range** reconcile to the next schedule boundary, upcoming schedule create/edit/move/delete, Delete unused vs End, started/history visibility when live audience excludes a person, atomic lifecycle receipts, read-first Routines UX, Today | Routines | Household navigation, and shared Calm Household presentation (migrations `007_routine_schedule_lifecycle.sql` and repair `008_widen_routine_mutation_receipt_kinds.sql` for receipt `kind` CHECK). r2 locks, personal tomorrow-floor, group next-day, sync/outbox, and Origin/CSRF/WS protections are preserved. Recorded pre-P0-005 integrated baseline: `93ef494`.
...
The r1/r2 foundation is on integrated `main`. Local r3 implements the authoritative contract in `briefs/p0-005-multiple-household-routines.md` revision 3. D-020/D-021 retain identity/daypart conventions; D-024–D-026 describe the r3 lifecycle and shell. Older milestone narratives below are historical context where a later decision explicitly refines them.
...
### P0-005 r3 routine lifecycle and application design (implemented locally)

D-024–D-026 and brief revision 3 are implemented on the r3 branch; Architecture technical acceptance of the Build Report and Project Lead/product acceptance remain pending. Prior r1/r2 acceptance is not evidence for these behaviors on `main` until merge.
