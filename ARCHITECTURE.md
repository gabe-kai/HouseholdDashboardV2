...
- **Current repository truth:** Integrated `main` at `7cba3a6` contains accepted **P0-006A r2** via PR #12, the A toast/CI correction, and B planning. The Project Lead accepted A after screenshots and local use. **P0-006B r1** contextual applicability is implemented on `brief/p0-006b-contextual-routine-applicability` (readiness `25c3c48`); Architecture returned FIX REQUIRED for AT9 rollback injection and AT10/AT11 browser multi-context evidence.
