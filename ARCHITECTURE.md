...
- **Current repository truth:** Integrated `main` is `51322e0` (PR #15), containing technically accepted **P0-007A r1** through correction `bb21d74` and acceptance writeback `7539fd0`, migrations 011-013, and the earlier accepted P0-006 foundation. The new Design handoff retains A as the responsibility foundation and carries its presentation findings to C. **P0-007B r1** is now the planned current slice; patterns, scheduled additions and Unassigned are not implemented at this baseline.
...
- **Composition:** Versioned named additions have weekdays, steps and inherited/fixed/cyclic ownership. Base plus applicable additions form one snapshotted occurrence; an addition's owner governs the whole day. Reject overlapping owner-setting additions rather than assigning hidden priority. The parent plan lifecycle controls prospective removal/editing without rewriting history.
- **Read/lock boundary:** A shared dated resolver feeds saved/draft previews, materialization, reconciliation and first-action checks. Preview has no writes. Started/past snapshots remain authoritative; unstarted today/future rows reconcile in their governed ranges, including dated group changes. Structural identity covers effective owner/work/provenance independently of plan version alone.
- **Unassigned:** Extend A's NOT NULL owner and migration 013 trigger through forward migration(s), permitting null only for unstarted responsibilities. Keep same-household assigned owners, routine required owners and both uniqueness rules. Show unresolved work in Plan/Household and stored History; no personal executor or fabricated identity.
- **Compatibility/evidence:** Audit legacy commands/receipts, group references, cleanup, dirty drafts, sync and reset retention. Add populated through-013 upgrade evidence; retain existing baselines. Required normal UI Kitchen/Bathroom/Cats/Trash journeys, date/race/rollback/offline/source-change cases and exact local PR/RC/Vite gates. New fictional screenshots only under `reports/p0-007b-r1-screenshots/`; stable names and opt-in fixtures, no live-household seeding or hosted iteration.
