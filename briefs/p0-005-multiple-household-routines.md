...
### 1. Structural lock and editable occurrences

- An occurrence is unstarted until the first committed checklist execution action for that occurrence. Viewing, loading, expanding, switching devices, or materializing it does not start or lock it.
- The first committed action of any valid checklist status—Required completed, Optional completed, As-needed completed, or As-needed Not needed—monotonically locks that occurrence's structure. Undoing the action does not unlock it. A started but incomplete occurrence remains locked.
- The lock belongs to one routine/person/date occurrence. People receiving the same group-backed routine may have different snapshots on the same date: an unstarted occurrence can update while a started occurrence remains unchanged.
- A structural edit may reconcile an applicable unstarted occurrence for today or an explicitly chosen future start date. It may change title, steps, order, obligations, daypart, audience, schedule, or applicable personal content according to authority and date rules. It must never partially merge fields from different structures.
- Once an occurrence is started, structural edits must not change its title, daypart, steps, obligations, audience/accountable identity, personal composition, or reports. Later changes apply to the next eligible occurrence or an explicitly selected later date. Completed and historical occurrences remain immutable.
- The first execution action and a concurrent structural edit resolve atomically. Either the action commits against the updated structure and locks it, or the edit leaves the old occurrence locked and becomes effective later. There must be no mixed snapshot. A client with a pending first action treats that occurrence as locally started for reconciliation and never knowingly applies a later structural refresh over it.
- The UI speaks in household terms such as “This routine hasn’t been started yet” and “Daniel has already started this routine”; it does not expose revision IDs or database lock terminology. Save feedback identifies who changes today, who is already protected, and the chosen future start date where applicable.

### 2. Independent definitions and dated configuration
...
- Initial creation is effective today in the household timezone and appears on Today immediately if its weekday and audience apply. An edit may apply today to applicable unstarted occurrences. For an unstarted future occurrence or deliberately chosen future start, repeated edits continue refining that same intended date; an existing revision at that date is not a reason to drift to a later date. Once affected occurrences are started, later changes become future-effective. Show the intended date and expected affected people before saving and return the actual result for feedback/preview. Changes to another routine must not occupy this routine's date or delay its save. A concurrency conflict must not silently reschedule the user's change.
...
### 4. Focused routine authoring and discovery
...
### 5. People/group participation and references
...
### 6. Routine-scoped personalization and approvals
...
### 7. Independent Today, activity, and history
...
- Today uses each occurrence's current eligible structure for unstarted work and its immutable snapshot after the first execution action, for labels, daypart ordering, obligation/completion state, expansion, pending commands, and errors. Completing or reopening one occurrence cannot change another routine, even with identical step labels or ordering. Preserve rapid optimistic actions, membership-isolated durable outbox, local started protection, and authoritative version reconciliation.
...
- Preserve future materialization for deterministic evaluation. Future participation may include/exclude retained rows under the established group rule, but a retained unstarted occurrence may reconcile to the newest applicable structure while a started one remains frozen. Future status writes and excluded cached occurrence writes remain rejected without reports/receipts. A changed shared/personal definition never rewrites a started, completed, or historical snapshot; normal preview remains read-only.

### 8. Safe archive lifecycle
...
### 9. API, authorization, replay, and synchronization
...
- Update `docs/protected-behaviors.md` and its sync matrix, relevant operator migration instructions, and factual architecture notes. Add focused domain/integration/HTTP/browser evidence and fictional phone-width screenshots in `reports/p0-005-r2-screenshots/`; leave prior evidence artifacts untouched.
...
- Suggested branch: `brief/p0-005-multiple-household-routines`. Engineering readiness/report paths: `reports/P0-005-r2-engineering-readiness.md` and `reports/P0-005-r2-build-report.md`. Suggest commit messages but do not commit; the Project Lead manages Git operations.
...
| 8. Future rows and immutable history | Pre-materialize a future date, add/remove group participants, and verify authoritative results reconcile while surviving/retained rows, step snapshots, reports, and receipts are not rewritten. An unstarted current/future occurrence may absorb a whole-structure edit; a started occurrence and all history remain unchanged. Reject future and excluded cached writes. Change Bedtime then compare yesterday's three routines, including IDs and snapshot fields, unchanged. |
...
| 19. Execution-start structural lock | Create or open an applicable routine and make repeated corrections/additions for the same household date while it is unstarted. The first valid execution action (including Required/Optional complete and As-needed complete or Not needed) locks that occurrence; undo does not unlock it. Afterward, edits do not change the started occurrence, while the next eligible unstarted occurrence receives the change. Prove this independently for group-backed members on the same date, deliberate future dates, pending first-execution outbox intent, reconnect, and an edit/action race. |
...
- **r2:** Product-approved structural-lock contract: unstarted occurrences may absorb same-day or same-intended-date changes; the first committed execution action locks one occurrence permanently; group-backed members may lock independently; edit/execution races resolve atomically. New readiness is required.
