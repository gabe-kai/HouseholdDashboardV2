...
**Engineering status:** IMPLEMENTED (AT6 four-owner matrix evidence closed for Architecture acceptance)  
...
- AT6 Household e2e now proves the normal overview with four fictional owners (Avery/Casey/Jordan/Taylor) on Kitchen/Cats/Bathroom/Trash with distinct states, plus multi-person routine aggregates, before drill/manage.
...
- `tests/e2e/z-p0-007c-1-*.spec.ts` — AT2/AT3/AT5/AT6/AT11/AT12 journeys (AT6 four-owner matrix)
...
| `npm test` | PASS | 215 tests (also via prior validate) |
| `npm run typecheck` / `npm run lint` | PASS | via prior `validate` |
| `npx playwright test --project=chromium --grep "P0-007C-1"` | PASS | **8 passed** (incl. AT6 four-owner matrix) |
| `npx playwright test --project=webkit --grep "P0-007C-1"` | PASS | **7 passed, 1 skipped** (geometry skipped on WebKit by design; AT2 + AT6 matrix covered) |
| `npx playwright test --project=chromium tests/e2e/z-p0-007c-1-household.spec.ts` | PASS | AT5 + AT6 matrix |
| `npx playwright test --project=webkit tests/e2e/z-p0-007c-1-household.spec.ts` | PASS | AT5 + AT6 matrix |
| `npx playwright test --project=chromium-desktop tests/e2e/z-p0-007c-1-geometry.spec.ts` | PASS | AT12 geometry (prior) |
| `npx playwright test --project=chromium tests/e2e/morning-routine.spec.ts` | PASS | **12 passed** (prior) |
| `npm run validate:pr` | PASS | exit 0 prior to AT6 matrix extension; Chromium C-1 re-run PASS after extension |
| `npm run validate:rc` | PASS | exit 0 prior to AT6 matrix extension; WebKit C-1 re-run PASS after extension |
...
| 6 Household oversight | `z-p0-007c-1-household.spec.ts` AT6 Chromium + WebKit: Household root shows Kitchen→Avery Not started, Cats→Casey In progress, Bathroom→Jordan Complete, Trash→Taylor Not started; Morning `1/4` and After `0/2` people complete without opening checklists; then routine→person and responsibility drills + Manage links | PASS |
...
| 13 Regression/CI | Job `e2e-phone-007c1`, desktop allowlist, aggregate gate; local `validate:pr` + `validate:rc` PASS (prior); Chromium+WebKit C-1 greps re-PASS after AT6 matrix | PASS local; first remote Actions after Project Lead push |
...
- No other material contract deviations. Prior AT6 “focused path only” disclosure is **closed** by the four-owner Household matrix e2e.
...
```
fix(P0-007C-1): prove AT6 four-owner Household matrix in e2e

Extend Chromium/WebKit Household oversight to assert Kitchen/Cats/Bathroom/Trash
owners and states from the overview, and record committed implementation SHAs.
```
